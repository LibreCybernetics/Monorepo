...
data class UnsignedVarInt(val int: BigInteger) {
    companion object {
        ...
        fun encode(int: BigInteger): ByteArray {
            ...
            val bytes =
                    ByteArray(digits) { idx ->
                        (int.divide(128.toBigInteger().pow(idx))
                                        .mod(128.toBigInteger())
                                        .toInt()
                                        .toUByte() +
                                        if (idx == digits - 1) 0.toUByte() else 128.toUByte())
                                .toByte()
                    }
        ...
        fun decode(bytes: ByteArray): BigInteger {
            ...
            require(bytes.dropLast(1).all { it.toUByte() >= 128.toUByte() })
            require(bytes.last().toUByte() < 128.toUByte())

            return bytes
                    .mapIndexed { idx, byte ->
                        128.toBigInteger().pow(idx) *
                                byte.toUByte().mod(128.toUByte()).toInt().toBigInteger()
                    }
                    .reduce { a, b -> a + b }
        }
    }
